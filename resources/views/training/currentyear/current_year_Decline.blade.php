...
@section('page-title')
    {{__('Current Year Decline Training')}}
@endsection
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item">{{__('Current Year Decline Training')}}</li>
@endsection

@section('action-btn')
    <div class="float-end">
        <a href="{{ route('currentYearDecline.export') }}?company_id={{ request('company_id') }}&from_date={{ request('from_date') }}&to_date={{ request('to_date') }}&depot_id={{ request('depot_id') }}&group_id={{ request('group_id') }}" class="btn btn-success">{{ __('Export to Excel') }}</a>
...
<div class="row" style="margin-bottom: 10px;margin-top:10px;">
    <div class="col-12">
        <!-- Filter Form -->
        <form method="GET" action="{{ route('currentYearDecline') }}">
            <div class="row">
                ...
                <div class="col-md-4 mt-3">
                    ...
                    <a href="{{ route('currentYearDecline') }}" class="btn btn-secondary">{{__('Reset Filter')}}</a>
                </div>
            </div>
        </form>
    </div>
                ...
                <div class="card-body table-border-style">
                    <div class="table-responsive">
                        <table class="table datatable">
                           ...
                           @foreach ($DeclineTraining as $Decline)
                                <tr>
                                    <td style="text-align: left">{{ $Decline->driver->name ?? 'Driver Name Not Found' }}</td>
                                    <td style="text-align: left">{{ $Decline->driver->contact_email ?? null }}</td>
                                    <td style="text-align: left">{{ $Decline->driver->contact_no ?? null }}</td>
                                    <td style="text-align: left">{{ $Decline->driver->group->name ?? null }}</td>
                                    <td style="text-align: left">{{ $Decline->training->trainingType->name ?? null }}</td>
                                    <td style="text-align: left">{{ $Decline->training->trainingCourse->name ?? null }}</td> <!-- Updated to access training course -->
                                    <td style="text-align: left">{{ \Carbon\Carbon::parse($Decline->training->from_date)->format('d/m/Y') }}</td>
                                    <td style="text-align: left">{{ \Carbon\Carbon::parse($Decline->training->to_date)->format('d/m/Y') }}</td>
                                    <td style="text-align: left">{{ $Decline->status }}</td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
