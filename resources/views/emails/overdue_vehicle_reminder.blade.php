...
<body>
    ...
    $columns = [
        'taxDueDate' => 'Road Tax',
        'annual_test_expiry_date' => 'MOT',
        'tacho_calibration' => 'Tacho Calibration',
        'dvs_pss_permit_expiry' => 'DVS PSS Permit Expiry',
        'insurance' => 'Insurance',
        'date_of_inspection' => 'Date Of Inspection',
        'PMI_due' => 'PMI Due',
        'brake_test_due' => 'Brake Test Due'
    ];

    // Determine which columns have data
    $columnsToShow = [];
    foreach ($data['overdue'] as $reminder) {
        foreach ($columns as $key => $column) {
            if (isset($reminder[$key]) && $reminder[$key] !== '-' && $reminder[$key] !== null) {
                $columnsToShow[$key] = $column;
            }
        }
    }
    ?>
    ...
    <table border="1" cellpadding="5" cellspacing="0">
        ...
        <tbody>
            @foreach ($data['overdue'] as $registrationNumber => $reminder)
            <tr>
                <td>{{ $registrationNumber }}</td>
                @foreach ($columnsToShow as $key => $column)
                    <td>
                        @if (isset($reminder[$key]))
                            {{ $reminder[$key] }}
                            @if ($key === 'taxDueDate' && isset($reminder['taxStatus']))
                                <br>
                                ({{ $reminder['taxStatus'] ?? '-' }})
                            @endif
                        @else
