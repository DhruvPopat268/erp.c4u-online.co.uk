...
@push('script-page')
    ...
    <script>
        ...
        $('.summernote-simple').on('summernote.blur', function () {
            ...
            $.ajax({
                url: "{{route('silver.policy_description.store',$forsSilver->id)}}",
                data: {_token: $('meta[name="csrf-token"]').attr('content'), silver_policy_description: $(this).val()},
                type: 'POST',
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="{{ route('fors.silver.index') }}">{{ __('Silver Policy') }}</a></li>
    <li class="breadcrumb-item active" aria-current="page">{{$forsSilver->silver_policy_name}}</li>
@endsection
       ...
       <div id="useradd-1">
           ...
           <div class="card">
               <div class="card-header">
                   <h5 class="mb-0">{{ __('Silver Policy Description - ')  }} {{$forsSilver->silver_policy_name}}</h5>
               </div>
               <div class="card-body" >
                   <div class="col-md-12">
                       <div class="form-group mt-3" >
                           <textarea class="summernote-simple" >{!! $forsSilver->silver_policy_description !!}</textarea>
                       </div>
                   </div>
               </div>
           </div>
