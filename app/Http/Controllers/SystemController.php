...
class SystemController extends Controller
{
    public function index()
    {
        if (\Auth::user()->can('manage system settings')) {
            ...
            $driverVersion  = AppVersion::where('type', 'driver')->first();
$managerVersion = AppVersion::where('type', 'manager')->first();

            return view('settings.index', compact('settings', 'admin_payment_setting', 'file_size','driverVersion','managerVersion'));
        } else {
...
public function appVersionSettings(Request $request)
{
    AppVersion::updateOrCreate(
        ['type' => 'driver'],
        [
            'version' => $request->driver_version,
            'maintenance_mode' => $request->has('driver_maintenance')
        ]
    );
    ...
    AppVersion::updateOrCreate(
        ['type' => 'manager'],
        [
            'version' => $request->manager_version,
            'maintenance_mode' => $request->has('manager_maintenance')
        ]
    );
